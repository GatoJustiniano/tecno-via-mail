...
public class SalidaDato {
    ...
    public SalidaDato(String motivo) {
        ...
        // Utilizar SimpleDateFormat para obtener la fecha y la hora en el formato correcto
        SimpleDateFormat formatoFecha = new SimpleDateFormat("dd/MM/yyyy");
        SimpleDateFormat formatoHora = new SimpleDateFormat("HH:mm:ss");

        // Obtener la fecha y la hora actuales formateadas
        this.fecha = formatoFecha.format(new Date());
        this.hora = formatoHora.format(new Date());

        // Ajustar la hora restando 1 hora
        int horaActual = Integer.parseInt(this.hora.substring(0, 2)) - 1;
        if (horaActual < 0) {
            horaActual = 23; // Si la hora es negativa, ajustarla a 23 (hora anterior del día previo)
        }
        // Mantener el formato "HH:mm:ss"
